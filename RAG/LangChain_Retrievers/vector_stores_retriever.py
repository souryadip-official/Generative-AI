...
vector_store = Chroma.from_documents( # It does both the creation of vector store and adding of the documents together
    ...
    collection_name='cricketer_sample'
)

retriever = vector_store.as_retriever(
    search_kwargs={'k': 3}, # Number of relevant documents we want to get back
) # Converting the vector store into a retriever -> by default, simple similarity-based retriever
user_query = 'Who among these is the most successful captain?'
docs = retriever.invoke(input=user_query)

for idx, doc in enumerate(docs):
    print(f'\n*-------- Result {idx+1} --------*\nContent: {doc.page_content}\n')
print('-'*10, 'END', '-'*10) # Here we face the problem of repetition of the same results thereby leading to redundancy